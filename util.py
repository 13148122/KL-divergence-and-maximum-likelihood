...
def plot_points(x, y, save_path):
    """Plot the test error vs. norm of the solution
    part (c) of Implicit Regularization

    Args:
        x: list of norms
        y: list of test errors
...
def plot_training_and_test_curves(logs, save_path, label):
    """Plot multiple training/test curves

    For better visualization, we add the following trick:
        1. we only plot one point every 10 steps

    Args:
        logs: list of (steps, training error, test error) tuple
        save_path: path to save the plot
    ...
    for i in range(len(logs)):
        log = logs[i]
        plt.plot(log[0][::10], log[2][::10], label = "test error, " + label[i])
    for i in range(len(logs)):
        log = logs[i]
