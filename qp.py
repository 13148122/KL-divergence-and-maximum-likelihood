...
class QP:
    ...
    def train_GD(self, X, Y, eta = 8e-2, max_step = 1000, 
        ...
        Args:
            ...
            X_val: validation/test input
            Y_val: validation/test output
        """
        if verbose:
            log_steps = []
            log_vals = []
        ...
        for t in range(max_step):
            ...
            for i in np.arange(n):
                grad_theta += (X[i, :].dot(self.theta ** 2 - self.phi ** 2) - Y[i]) * np.multiply(self.theta, X[i,:])
                grad_phi += (X[i, :].dot(self.theta ** 2 - self.phi ** 2) - Y[i]) * np.multiply(self.phi, X[i, :])
            ...
            if verbose:
                log_steps.append(t)
                log_vals.append(self.test(X_val, Y_val))
    ...
    def train_SGD(self, X, Y, eta = 8e-2, max_step = 1000, batch_size = 1, 
        ...
        for t in range(max_step):
            ...
            for i in np.arange(n):
                grad_theta += (X_batch[i, :].dot(self.theta ** 2 - self.phi ** 2) - Y_batch[i]) * np.multiply(self.theta, X_batch[i, :])
                grad_phi += (X_batch[i, :].dot(self.theta ** 2 - self.phi ** 2) - Y_batch[i]) * np.multiply(self.phi, X_batch[i, :])
            grad_theta = grad_theta/n
            grad_phi = -grad_phi/n
...
def QP_model_initialization(train_path, valid_path):
    ...
    for i in range(len(alphas)):
        ...
        print("initialization: {:.3f}".format(alpha), "final test error: ", model.test(X_val, Y_val))
...
def QP_model_batchsize(train_path, valid_path):
    ...
    for i in range(len(bs)):
        ...
        print("batchsize: ", bs[i], "final test error: ", model.test(X_val, Y_val))
    util.plot_training_and_test_curves(log, save_path, label = labels)
